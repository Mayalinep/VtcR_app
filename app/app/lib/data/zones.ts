/**
 * Zones Data - Liste des zones desservies par VTC Rachel
 * 
 * Zones géographiques couvertes en Île-de-France.
 * Affichées sur la landing page et potentiellement ailleurs.
 */

export const ZONES_DATA: string[] = [
  'Paris intra-muros',
  'Aéroport CDG',
  'Aéroport Orly',
  'La Défense',
  'Versailles',
  'Saint-Denis',
  'Neuilly',
  'Boulogne',
];

/**
 * Texte de description des zones
 */
export const ZONES_DESCRIPTION = 'Nous couvrons l\'ensemble de l\'Île-de-France';

/**
 * Texte additionnel affiché sous la liste
 */
export const ZONES_ADDITIONAL_TEXT = 'Et bien d\'autres destinations en Île-de-France';
